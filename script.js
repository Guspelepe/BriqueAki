...
window.limparFiltros = function() {
    ...
    document.getElementById('searchRaio').value = '0';
    document.querySelectorAll('.category-item').forEach(item => item.classList.remove('active'));
    const firstCat = document.querySelector('.category-item'); if (firstCat) firstCat.classList.add('active');
    renderizarAnuncios(); showToast('✅ Filtros limpos!');
};
...
async function renderAdminPanel() {
    ...
    const trocas = await db.trocas.toArray(); document.getElementById('adminTotalTrocas').textContent = trocas.length;
...
async function solicitarTroca(id) {
    ...
    const produto = allProducts.find(p => p.id === id); if (!produto) return;
    ...
    const mensagem = prompt('💬 Envie uma mensagem para o vendedor:\n\nDescreva o que você oferece em troca:'); if (mensagem === null) return; if (!mensagem.trim()) { showToast('❌ Por favor, digite uma mensagem.'); return; }
    const novaTroca = { produtoId: produto.id, produtoTitulo: produto.titulo, solicitante: currentUser.email, dono: produto.dono, status: 'pendente', tipo: produto.tipoTroca || 'produto', mensagem: mensagem.trim(), data: new Date().toISOString(), dataEntrega: null, dataRecebimento: null, avaliado: false, localCombinado: null };
    await db.trocas.add(novaTroca);
    await db.notificacoes.add({ para: produto.dono, de: currentUser.email, produtoId: produto.id, produtoTitulo: produto.titulo, mensagem: `${currentUser.nome} quer trocar "${produto.titulo}"\n\nMensagem: ${mensagem.trim()}`, data: new Date().toISOString(), lida: false, tipo: 'solicitacao_troca' });
    showToast(`✅ Solicitação enviada para ${produto.dono}!`); atualizarBadge(); renderMinhasTrocas();
}
...
async function responderTroca(trocaId, resposta) {
    ...
    renderMinhasTrocas(); renderizarAnuncios(); atualizarBadge();
}

async function marcarEntregue(trocaId) {
    const troca = await db.trocas.get(trocaId); if (!troca) return;
    troca.status = 'entregue'; troca.dataEntrega = new Date().toISOString();
    await db.trocas.put(troca);
    ...
    showToast('📦 Produto marcado como entregue!'); renderMinhasTrocas(); atualizarBadge();
}

async function marcarRecebido(trocaId) {
    const troca = await db.trocas.get(trocaId); if (!troca) return;
    troca.status = 'concluida'; troca.dataRecebimento = new Date().toISOString();
    await db.trocas.put(troca);

    const produto = await db.produtos.get(troca.produtoId);
    if (produto) {
        const preco = produto.precoMoedas; const taxa = Math.floor(preco * 0.15); const valorFinal = preco - taxa;
        const usuarios = await db.usuarios.toArray();
        ...
        produto.status = 'vendido'; produto.vendido = true; await db.produtos.put(produto); await loadProducts();
    }
    ...
    showToast('✅ Troca concluída com sucesso!'); renderMinhasTrocas(); renderizarAnuncios(); atualizarMoedas(); atualizarBadge(); abrirAvaliacaoModal(trocaId);
}
...
async function renderMinhasTrocas() {
    const minhasTrocasList = document.getElementById('minhasTrocasList'); if (!minhasTrocasList) return;
    ...
    if (minhas.length === 0) { minhasTrocasList.innerHTML = '<p style="color:#94a3b8;">Você ainda não tem trocas.</p>'; return; }
    minhasTrocasList.innerHTML = minhas.sort((a, b) => new Date(b.data) - new Date(a.data)).map(t => {
        const isSolicitante = t.solicitante === currentUser.email; const outroUsuario = isSolicitante ? t.dono : t.solicitante;
        const statusLabel = getTrocaStatusLabel(t.status); const statusClass = getTrocaStatusClass(t.status);
        const podeAceitar = t.status === 'pendente' && t.dono === currentUser.email; const podeEntregar = t.status === 'aceita' && t.dono === currentUser.email;
        const podeReceber = t.status === 'entregue' && t.solicitante === currentUser.email; const podeAvaliar = t.status === 'concluida' && !t.avaliado;
        return `<div class="troca-item" data-troca-id="${t.id}"><div class="troca-info"><div class="produto-nome">📦 ${escapeHtml(t.produtoTitulo)}</div><div style="font-size:0.85rem;color:#64748b;">${isSolicitante ? 'Você quer' : 'Você recebeu'} de ${escapeHtml(outroUsuario)}</div><div style="font-size:0.85rem;color:#64748b;margin-top:4px;">💬 ${escapeHtml(t.mensagem)}</div>${t.localCombinado ? `<div style="font-size:0.85rem;color:#16a34a;margin-top:4px;">📍 Local combinado: ${escapeHtml(t.localCombinado)}</div>` : ''}<span class="troca-status ${statusClass}">${statusLabel}</span>${t.dataEntrega ? `<div style="font-size:0.75rem;color:#94a3b8;">Entregue em: ${new Date(t.dataEntrega).toLocaleDateString('pt-BR')}</div>` : ''}${t.dataRecebimento ? `<div style="font-size:0.75rem;color:#94a3b8;">Recebido em: ${new Date(t.dataRecebimento).toLocaleDateString('pt-BR')}</div>` : ''}</div><div class="troca-actions">${podeAceitar ? `<button class="btn-aceitar-troca" data-action="aceitar-troca" data-id="${t.id}">✅ Aceitar</button><button class="btn-recusar-troca" data-action="recusar-troca" data-id="${t.id}">❌ Recusar</button>` : ''}${podeEntregar ? `<button class="btn-entregar" data-action="marcar-entregue" data-id="${t.id}">📦 Marcar Entregue</button>` : ''}${podeReceber ? `<button class="btn-receber" data-action="marcar-recebido" data-id="${t.id}">✅ Confirmar Recebimento</button>` : ''}${podeAvaliar ? `<button class="btn-avaliar" data-action="avaliar" data-id="${t.id}">⭐ Avaliar</button>` : ''}<button class="btn-chat" data-action="chat" data-id="${t.produtoId}" data-dono="${outroUsuario}">💬 Chat</button></div></div>`;
    }).join('');
}
...
async function enviarAvaliacao(trocaId) {
    ...
    fecharAvaliacaoModal(); showToast('⭐ Avaliação enviada! Obrigado!'); renderAvaliacoes(); atualizarBadge();
}
...
function abrirChatExistente(chatId, produtoId) {
    const modal = document.getElementById('chatModal'); const messages = document.getElementById('chatMessages'); const title = document.getElementById('chatTitle'); if (!modal || !messages || !title) return;
    db.chats.get(chatId).then(chat => {
        if (!chat) return;
        const produto = allProducts.find(p => p.id === produtoId); title.textContent = `Conversa sobre: ${produto ? produto.titulo : 'Produto'}`;
        let localSection = ''; if (chat.localCombinado) { localSection = `<div class="chat-local-combinado ${chat.localAceito ? 'aceito' : ''}">📍 Local combinado: ${escapeHtml(chat.localCombinado)} ${chat.localAceito ? '✅ Aceito' : '<button onclick="aceitarLocalCombinado(' + chatId + ')">✅ Aceitar local</button>'}</div>`; }
        messages.innerHTML = localSection + chat.mensagens.map(msg => { const isMe = msg.de === currentUser.email; const data = new Date(msg.data).toLocaleDateString('pt-BR'); const hora = new Date(msg.data).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }); return `<div class="chat-message ${isMe ? 'me' : 'other'}"><strong>${isMe ? 'Você' : escapeHtml(msg.de)}</strong><br />${escapeHtml(msg.texto)}<span class="msg-date">${data} ${hora}</span></div>`; }).join('');
        messages.scrollTop = messages.scrollHeight; modal.classList.add('active'); chatIdAtual = chatId;
    });
}
...
async function aceitarLocalCombinado(chatId) {
    try {
        const chat = await db.chats.get(chatId); if (!chat) return;
        chat.localAceito = true; await db.chats.put(chat);
        showToast('✅ Local aceito!'); const outro = chat.usuario1 === currentUser.email ? chat.usuario2 : chat.usuario1;
        ...
        atualizarBadge(); abrirChatExistente(chatId, chat.produtoId);
    } catch (error) { console.error('Erro ao aceitar local:', error); showToast('❌ Erro ao aceitar local.'); }
}

async function combinarLocal() {
    const inputLocal = document.getElementById('chatLocalInput'); if (!inputLocal || !chatIdAtual) return; const local = inputLocal.value.trim(); if (!local) return;
    try {
        const chat = await db.chats.get(chatIdAtual); if (!chat) return;
        chat.localCombinado = local; chat.localAceito = false;
        ...
        showToast('📍 Local sugerido!'); inputLocal.value = ''; abrirChatExistente(chatIdAtual, chat.produtoId); atualizarBadge();
    } catch (error) { console.error('Erro ao combinar local:', error); showToast('❌ Erro ao combinar local.'); }
}

async function enviarMensagem() {
    const input = document.getElementById('chatInput'); if (!input || !chatIdAtual) return; const texto = input.value.trim(); if (!texto) return;
    try {
        const chat = await db.chats.get(chatIdAtual); if (!chat) return;
        ...
        atualizarBadge(); abrirChatExistente(chatIdAtual, chat.produtoId);
...
async function renderChats() {
    const container = document.getElementById('chatListContainer'); if (!container) return;
    ...
    try {
        ...
        minhas.sort((a, b) => { const aLast = a.mensagens.length > 0 ? new Date(a.mensagens[a.mensagens.length - 1].data) : new Date(a.data); const bLast = b.mensagens.length > 0 ? new Date(b.mensagens[b.mensagens.length - 1].data) : new Date(b.data); return bLast - aLast; });
        let html = ''; for (const chat of minhas) { const outro = chat.usuario1 === currentUser.email ? chat.usuario2 : chat.usuario1; const ultimaMsg = chat.mensagens.length > 0 ? chat.mensagens[chat.mensagens.length - 1] : null; const dataMsg = ultimaMsg ? new Date(ultimaMsg.data) : new Date(chat.data); const dataFormatada = dataMsg.toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }); const produto = allProducts.find(p => p.id === chat.produtoId); const nomeProduto = produto ? produto.titulo : 'Produto desconhecido'; html += `<div class="chat-list-item" data-chat-id="${chat.id}" data-produto-id="${chat.produtoId}" data-outro="${outro}"><div class="chat-info"><div class="chat-with">👤 ${escapeHtml(outro)}</div><div class="chat-product">📦 ${escapeHtml(nomeProduto)}</div>${chat.localCombinado ? `<div class="chat-local-status">📍 ${escapeHtml(chat.localCombinado)} ${chat.localAceito ? '✅' : '⏳'}</div>` : ''}${ultimaMsg ? `<div class="chat-last-msg">${escapeHtml(ultimaMsg.texto)}</div>` : ''}</div><div class="chat-date">${dataFormatada}</div></div>`; } container.innerHTML = html;
...
function renderNotificacoes() {
    ...
    db.notificacoes.toArray().then(notifs => {
        ...
        notificationsList.innerHTML = minhas.sort((a, b) => new Date(b.data) - new Date(a.data)).map(n => { const data = new Date(n.data).toLocaleDateString('pt-BR'); const hora = new Date(n.data).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }); let botoes = ''; if (n.tipo === 'solicitacao_troca' && !n.lida) botoes = `<button class="btn-aceitar" onclick="showTab('minhasTrocas')">Ver Trocas</button>`; if (n.tipo === 'troca_aceita' && !n.lida) botoes = `<button class="btn-chat-notif" onclick="showTab('minhasTrocas')">Ver Trocas</button>`; if (n.tipo === 'chat' && !n.lida) botoes = `<button class="btn-chat-notif" onclick="abrirChat(${n.produtoId}, '${n.de}')">Responder</button>`; if (n.tipo === 'avaliacao' && !n.lida) botoes = `<button class="btn-chat-notif" onclick="showTab('avaliacao')">Ver Avaliações</button>`; if (n.tipo === 'local_combinado' && !n.lida) botoes = `<button class="btn-chat-notif" onclick="showTab('chats')">Ver Chat</button>`; if (n.tipo === 'local_aceito' && !n.lida) botoes = `<button class="btn-chat-notif" onclick="showTab('chats')">Ver Chat</button>`; return `<div class="notification-item" style="${n.lida ? 'opacity:0.6;' : ''}"><div class="notif-text"><strong>${escapeHtml(n.de)}</strong><br />${escapeHtml(n.mensagem).replace(/\n/g, '<br>')}</div><div style="display:flex;align-items:center;flex-wrap:wrap;gap:5px;"><span class="notif-date">${data} ${hora}</span>${botoes}</div></div>`; }).join('');
    });
}
...
function logoutUser() { currentUser = null; localStorage.removeItem('currentUser'); const notificationsPanel = document.getElementById('notificationsPanel'); if (notificationsPanel) notificationsPanel.classList.remove('active'); const prodModal = document.getElementById('productModal'); const chatModal = document.getElementById('chatModal'); if (prodModal) prodModal.classList.remove('active'); if (chatModal) chatModal.classList.remove('active'); atualizarHeader(); renderizarAnuncios(); renderMeusAnuncios(); renderAdminPanel(); renderMinhasTrocas(); showToast('👋 Você saiu.'); }

function atualizarHeader() { const userInfoLoggedIn = document.getElementById('userInfoLoggedIn'); const userInfoLoggedOut = document.getElementById('userInfoLoggedOut'); const displayName = document.getElementById('displayName'); const coinsDisplay = document.getElementById('coinsDisplay'); const notifBadge = document.getElementById('notifBadge'); const adminBtn = document.getElementById('tabAdminBtn'); if (currentUser) { if (userInfoLoggedIn) userInfoLoggedIn.style.display = 'flex'; if (userInfoLoggedOut) userInfoLoggedOut.style.display = 'none'; if (displayName) displayName.textContent = currentUser.nome; atualizarMoedas(); atualizarBadge(); const localProduto = document.getElementById('localProduto'); if (localProduto) localProduto.value = `${currentUser.cidade}/${currentUser.estado}`; if (adminBtn) adminBtn.style.display = currentUser.isAdmin ? 'inline-block' : 'none'; atualizarIndicacao(); } else { if (userInfoLoggedIn) userInfoLoggedIn.style.display = 'none'; if (userInfoLoggedOut) userInfoLoggedOut.style.display = 'flex'; if (coinsDisplay) coinsDisplay.textContent = '🪙 0'; if (notifBadge) { notifBadge.textContent = '0'; notifBadge.style.display = 'none'; } if (adminBtn) adminBtn.style.display = 'none'; } }
...
async function atualizarBadge() { const notifBadge = document.getElementById('notifBadge'); if (!currentUser) { if (notifBadge) { notifBadge.textContent = '0'; notifBadge.style.display = 'none'; } return; } const notificacoes = await db.notificacoes.toArray(); const naoLidas = notificacoes.filter(n => n.para === currentUser.email && !n.lida); if (notifBadge) { notifBadge.textContent = naoLidas.length; notifBadge.style.display = naoLidas.length > 0 ? 'inline' : 'none'; } }
...
// EVENT DELEGATION
// ============================================================
function setupEventDelegation() { document.addEventListener('click', async function(e) { const btn = e.target.closest('button[data-action]'); if (!btn) return; const action = btn.dataset.action; const id = parseInt(btn.dataset.id); e.stopPropagation(); switch(action) { case 'trocar': await solicitarTroca(id); break; case 'excluir': await excluirAnuncio(id); break; case 'chat': const dono = btn.dataset.dono; await abrirChat(id, dono); break; case 'aceitar-troca': await responderTroca(id, 'aceitar'); break; case 'recusar-troca': await responderTroca(id, 'recusar'); break; case 'marcar-entregue': await marcarEntregue(id); break; case 'marcar-recebido': await marcarRecebido(id); break; case 'avaliar': abrirAvaliacaoModal(id); break; } }); document.addEventListener('click', function(e) { const card = e.target.closest('.card'); if (card && !e.target.closest('button')) { const produtoId = card.dataset.produtoId; if (produtoId) window.location.href = `produto.html?id=${produtoId}`; } }); document.addEventListener('click', function(e) { const chatItem = e.target.closest('.chat-list-item'); if (chatItem && !e.target.closest('button')) { const produtoId = chatItem.dataset.produtoId; const outro = chatItem.dataset.outro; if (produtoId && outro) abrirChat(parseInt(produtoId), outro); } }); }

// ============================================================
// EVENT LISTENERS
// ============================================================
function setupEventListeners() { const btnLogin = document.getElementById('btnLogin'); if (btnLogin) btnLogin.addEventListener('click', () => loginUser(document.getElementById('loginEmail').value.trim(), document.getElementById('loginSenha').value)); document.getElementById('loginEmail')?.addEventListener('keypress', (e) => { if (e.key === 'Enter') btnLogin?.click(); }); document.getElementById('loginSenha')?.addEventListener('keypress', (e) => { if (e.key === 'Enter') btnLogin?.click(); }); const btnRegister = document.getElementById('btnRegister'); if (btnRegister) btnRegister.addEventListener('click', registerUser); const btnLogout = document.getElementById('btnLogout'); if (btnLogout) btnLogout.addEventListener('click', logoutUser); const btnPublicar = document.getElementById('btnPublicar'); if (btnPublicar) btnPublicar.addEventListener('click', publicarAnuncio); const btnNotifications = document.getElementById('btnNotifications'); if (btnNotifications) btnNotifications.addEventListener('click', toggleNotifications); const btnSearch = document.getElementById('btnSearch'); if (btnSearch) btnSearch.addEventListener('click', renderizarAnuncios); document.getElementById('searchText')?.addEventListener('keypress', (e) => { if (e.key === 'Enter') renderizarAnuncios(); }); const btnLocalizar = document.getElementById('btnLocalizar'); if (btnLocalizar) btnLocalizar.addEventListener('click', obterLocalizacao); const searchRaio = document.getElementById('searchRaio'); if (searchRaio) searchRaio.addEventListener('change', renderizarAnuncios); const searchEstadoEl = document.getElementById('searchEstado'); if (searchEstadoEl) searchEstadoEl.addEventListener('change', renderizarAnuncios); document.getElementById('searchCidade')?.addEventListener('keypress', (e) => { if (e.key === 'Enter') renderizarAnuncios(); }); const btnUsarCodigo = document.getElementById('btnUsarCodigo'); if (btnUsarCodigo) { btnUsarCodigo.addEventListener('click', async () => { if (!currentUser) { showToast('Faça login para usar um código.'); return; } const inputUsarCodigo = document.getElementById('inputUsarCodigo'); const codigo = inputUsarCodigo.value.trim().toUpperCase(); if (!codigo) { showToast('Digite um código.'); return; } if (codigo === currentUser.codigoIndicacao) { showToast('❌ Você não pode usar seu próprio código.'); return; } const usuarios = await db.usuarios.toArray(); const indicador = usuarios.find(u => u.codigoIndicacao === codigo); if (!indicador) { showToast('❌ Código inválido.'); return; } if (currentUser.indicadoPor) { showToast('❌ Você já usou um código de indicação.'); return; } const indicacoes = await db.indicacoes.toArray(); const usado = indicacoes.find(i => i.usadoPor === currentUser.email); if (usado) { showToast('❌ Você já usou um código de indicação.'); return; } currentUser.indicadoPor = indicador.email; currentUser.moedas += 20; indicador.moedas += 20; await db.usuarios.put(currentUser); await db.usuarios.put(indicador); await db.indicacoes.add({ codigo: codigo, criadoPor: indicador.email, usadoPor: currentUser.email, bonusRecebido: 20, dataUso: new Date().toISOString() }); showToast(`🎉 Código válido! Você e ${indicador.nome} ganharam 20 moedas!`); atualizarMoedas(); atualizarIndicacao(); inputUsarCodigo.value = ''; }); } const authModal = document.getElementById('authModal'); if (authModal) authModal.addEventListener('click', (e) => { if (e.target === authModal) fecharAuthModal(); }); const avaliacaoModal = document.getElementById('avaliacaoModal'); if (avaliacaoModal) avaliacaoModal.addEventListener('click', (e) => { if (e.target === avaliacaoModal) fecharAvaliacaoModal(); }); const btnEnviarAvaliacao = document.getElementById('btnEnviarAvaliacao'); if (btnEnviarAvaliacao) btnEnviarAvaliacao.addEventListener('click', () => { if (avaliacaoParaTrocaId) enviarAvaliacao(avaliacaoParaTrocaId); }); }
...
async function init() {
    ...
    const saved = localStorage.getItem('currentUser'); if (saved) { try { const data = JSON.parse(saved); const usuarios = await db.usuarios.toArray(); const user = usuarios.find(u => u.email === data.email); if (user) { currentUser = user; atualizarHeader(); renderMeusAnuncios(); renderAdminPanel(); renderMinhasTrocas(); renderAvaliacoes(); atualizarBadge(); renderChats(); atualizarIndicacao(); } } catch (e) { console.error('Erro ao restaurar sessão:', e); } }
    const params = new URLSearchParams(window.location.search); const tabParam = params.get('tab'); if (tabParam) { const map = { 'anuncios': 'anuncios', 'meusAnuncios': 'meusAnuncios', 'novoAnuncio': 'novoAnuncio', 'minhasTrocas': 'minhasTrocas', 'avaliacao': 'avaliacao', 'chats': 'chats', 'indicacao': 'indicacao', 'admin': 'admin' }; const target = map[tabParam.toLowerCase()]; if (target) showTab(target); }
    console.log('✅ Inicialização concluída (IndexedDB).');
}

// Expõe funções globais
window.showTab = showTab; window.abrirAuthModal = abrirAuthModal; window.fecharAuthModal = fecharAuthModal; window.copiarCodigo = copiarCodigo; window.fecharChat = fecharChat; window.aceitarLocalCombinado = aceitarLocalCombinado; window.combinarLocal = combinarLocal; window.enviarMensagem = enviarMensagem; window.fecharAvaliacaoModal = fecharAvaliacaoModal; window.abrirAvaliacaoModal = abrirAvaliacaoModal; window.fecharMapa = fecharMapa; window.limparLocalStorage = limparLocalStorage;
